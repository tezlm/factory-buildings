var _this = this;
var script = "[" + modName + "/" + scriptName + "] ";
var $ = function (name, type) {
    return Vars.content.getByName(ContentType[type || "block"], _this.modName + "-" + name);
};
$ = Object.assign($, {
    events: {
        events: {},
        ...
        create: function (event, func) {
            ...
            Events.on(event, function () {
                if (!_this.events.hasOwnProperty(id))
                    return;
                _this.events[id](_this.events[id]);
            });
            return id;
        },
        remove: function (id) {
    ...
    insertAfter: function (index, obj) {
        ...
        blocks.filter(function (i) { return i != obj; });
        blocks.insert((blocks.indexOf(index) || 0) + 1, obj);
        obj.category = index.category;
    },
    keys: function (obj) { return Object.keys(obj).toString(); },
    timer: {
    ...
    format: function (name) { return _this.modName + "-" + name; },
    console: {
        log: function (text) {
        ...
        trace: function () {
            var obj = {};
            Error.captureStackTrace(obj, _this.trace);
